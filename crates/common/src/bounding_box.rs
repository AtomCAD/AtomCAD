...
/// An axis-aligned bounding box defined by two opposite corners ([`min`](BoundingBox::min) and
/// [`max`](BoundingBox::max)).  `min.x <= max.x`, `min.y <= max.y`, `min.z <= max.z`.
...
impl BoundingBox {
    ...
    pub fn ray_hit_times(&self, origin: Vec3, velocity: Vec3) -> Option<(f32, f32)> {
        ...
        // If the ray exits the box before it enters it, then our assumption that the ray intersects
        // with the box is wrong and we return None.
        if tmin > tmax {
            return None;
        }
